    ...
    public function run() {
      ...
      switch ($this->getRequest('msgtype')) {
              ...
              switch ($this->getRequest('EventKey'))
                {
                    ...
                    case "chuangyexinxi":
                  	$q = "SELECT * FROM chuangyexinxi where id = 1";                  //SQL查询语句 
                    ...
                    while($row = mysql_fetch_row($rs)) 
                        ...
                        $chuangye = array(
        new NewsResponseItem($row[1], $row[2],$row[3], $row[4]),
      );
                    $this->responseNews($chuangye);
                  ...
                  $q = "SELECT * FROM jiuyexinxi where id =1";                  //SQL查询语句 
                    mysql_query("SET NAMES UTF-8");          
                    $rs = mysql_query($q, $con);                     //获取数据集 
                    if(!$rs)
                    {
        ...
        new NewsResponseItem($row[1], $row[2], $row[3], $row[4]),
                    ...
                    $this->responseNews($jiuye);
                        break;
                    
                  
                  case "xueyuandongtai":
                  
                  $q = "SELECT * FROM xueyuandongtai where id =1";                  //SQL查询语句 
                    mysql_query("SET NAMES UTF-8");          
                    $rs = mysql_query($q, $con);                     //获取数据集 
                    if(!$rs)
                    {
        ...
        new NewsResponseItem($row[1], $row[2], $row[3], $row[4]),
                  ...
                  $q = "SELECT * FROM hangyedongtai where id =1";                  //SQL查询语句 
                    mysql_query("SET NAMES UTF-8");          
                    $rs = mysql_query($q, $con);                     //获取数据集 
                    if(!$rs)
                    {
        ...
        new NewsResponseItem($row[1], $row[2], $row[3], $row[4]),
                    ...
                    case "xuexidongtai":
                  /*  $name = '崔雅迪';
                  $q = "SELECT * FROM student where 姓名 = '".$name."'";                  //SQL查询语句 
                    mysql_query("SET NAMES UTF-8");          
                    $rs = mysql_query($q, $con);                     //获取数据集 
                    if(!$rs)
                    {
                ...
                $this->responseText($contentStr);
                  ...
                  $q = "SELECT * FROM xuexidongtai where id =1";                  //SQL查询语句 
                    mysql_query("SET NAMES UTF-8");          
                    $rs = mysql_query($q, $con);                     //获取数据集 
                    if(!$rs)
                    {
        ...
        new NewsResponseItem($row[1], $row[2], $row[3], $row[4]),
                    ...
                    case "bantuanganbu":
                     $items = array(
        new NewsResponseItem('班长团支书', '联系方式', $this->getRequest('picurl'), 'http://sxaurjxy.sinaapp.com/src/bantuan.html'),
        new NewsResponseItem('学生会干部', '联系方式', $this->getRequest('picurl'), 'http://sxaurjxy.sinaapp.com/src/xueshenghui.html'),
      );
      ...
      $this->responseNews($items);
               break;
                    ...
                    case "fankuiyijian":
                      $contentStr = '反馈建议或意见请联系软件学院CSDN高校俱乐部，本微信接口主要以PHP代码编写。';
                $this->responseText($contentStr);
               break;

                  
                  
                  case "chaxuntongxunlu":
                  $biaozhi = 1;
                      $contentStr = '此功能正在测试中,请输入“姓名”加上要查询的姓名，例如：姓名高正炎';
                $this->responseText($contentStr);
               ...
               break;
                  ...
                  case "zaixianfanyi":
                      $contentStr = '请输入翻译加上需要翻译的内容，例如：翻译时间。';
                $this->responseText($contentStr);
               ...
               break;
                    ...
                    default:
                        ...
                        "Url" =>"weixin://addfriend/sxnydxrjxy");
                        break;
                }
                break;
